...
import {
  ...
  BadRequestException,
  Query,
  ForbiddenException, // Import thêm cái này
} from '@nestjs/common';
import { ApiBearerAuth, ApiBody, ApiOperation, ApiParam, ApiQuery, ApiResponse, ApiTags } from '@nestjs/swagger';
import { JwtAuthGuard } from 'src/auth/guards/jwt-auth.guard';
import { PostService } from './post.service';
...
export class PostController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Lấy tất cả bài viết (Chỉ dành cho Admin)' })
  async findAllAdmin(@Req() req: any) {
    // Kiểm tra quyền Admin ngay tại Controller
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Xóa bài viết (Chủ bài viết hoặc Admin mới xóa được)' })
  async remove(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    // Lấy ID và Role từ token của người đang gọi API
    const userId = req.user.id;
    const userRole = req.user.role; 

    // Truyền vào service để check quyền
