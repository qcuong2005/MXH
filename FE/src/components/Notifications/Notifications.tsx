...
// import { ensureConversation } from "@/services/message"; // 1. Import thêm service này
...
//   // 3. Xử lý click item (Đã sửa đổi logic tin nhắn)
//   const handleItemClick = async (notif: Notification) => {
//     if (!currentUser) return;

//     // --- A. Đánh dấu đã đọc (Giữ nguyên) ---
//     if (!notif.is_read) {
//       setNotifications((prev) =>
...
//     // TRƯỜNG HỢP: Thông báo tin nhắn
//     if (notif.type === NotificationType.NEW_MESSAGE) {
//       try {
//         // Gọi API lấy hội thoại (giống logic trong AllFriends)
//         const conversation = await ensureConversation(currentUser.token, notif.sender_id);

//         if (conversation?.id) {
//           // Lưu vào localStorage để trang /messages tự động mở đúng chat
...
//         } else {
//           console.error("Không tìm thấy conversation ID");
//           // Fallback: nếu lỗi thì vào profile
//           router.push(`/profile/${notif.sender_id}`);
//         }
//       } catch (error) {
//         console.error("Lỗi khi mở hội thoại từ thông báo:", error);
//         router.push(`/profile/${notif.sender_id}`);
//       }
//       return; // Kết thúc hàm, không chạy logic default bên dưới
//     }

//     // TRƯỜNG HỢP: Các thông báo khác (Giữ nguyên logic cũ)
