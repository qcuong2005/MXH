...
interface Props {
  ...
  onCloseDropdown?: () => void;      // Hàm đóng dropdown thông báo
  onOpenPost: (postId: number) => void; // Hàm callback để mở Modal ở cấp cha
}

export default function Notifications({ currentUser, onCloseDropdown, onOpenPost }: Props) {
  ...
  // --- HÀM HELPER: Lấy ID bài viết từ thông báo ---
  ...
  const groupedNotifications = useMemo(() => {
    ...
    sorted.forEach((notif) => {
      ...
      // Nếu là Like/Comment => Gộp theo Post ID để tránh spam
      if (
      ...
      if (!map.has(key)) {
        map.set(key, { ...notif, count: 1, groupedIds: [notif.id] });
      } else {
  ...
  // Socket: Lắng nghe thông báo mới realtime
  useEffect(() => {
  ...
  const handleItemClick = async (e: React.MouseEvent, groupNotif: GroupedNotification) => {
    e.stopPropagation(); // Ngăn sự kiện click lan ra ngoài làm đóng dropdown bất ngờ

    if (!currentUser) return;

    // A. Đánh dấu đã đọc (API)
    ...
    // 1. Nếu là LIKE/COMMENT -> Mở Modal bài viết (Không chuyển trang)
    if (
      ...
      if (postId) {
        onOpenPost(Number(postId)); // Gọi hàm cha để mở modal
        onCloseDropdown?.();        // Đóng dropdown thông báo
      }
      return;
    }

    // Các trường hợp sau đây đều chuyển trang -> Cần đóng dropdown
    onCloseDropdown?.(); 

    // 2. Nếu là LỜI MỜI KẾT BẠN -> Chuyển đến trang Friends tab Requests
    if (groupNotif.type === NotificationType.FRIEND_REQUEST) {
      router.push("/friends?view=requests");
      return;
    }

    // 3. Nếu là TIN NHẮN -> Chuyển đến trang Messages
    if (groupNotif.type === NotificationType.NEW_MESSAGE) {
      try {
        const conversation = await ensureConversation(currentUser.token, groupNotif.sender_id);
        if (conversation?.id) {
          localStorage.setItem("selectedConversationId", conversation.id.toString());
          localStorage.setItem("selectedFriendId", groupNotif.sender_id.toString());
    ...
    // 4. Các trường hợp còn lại (Follow, Accept Friend...) -> Về trang cá nhân người đó
    if (
  ...
  // Helper render icons & colors
  const renderIcon = (type: NotificationType) => {
     switch (type) {
      case NotificationType.NEW_LIKE: return <Heart className="w-3.5 h-3.5 text-white fill-current" />;
      case NotificationType.NEW_COMMENT: return <MessageCircle className="w-3.5 h-3.5 text-white fill-current" />;
      case NotificationType.NEW_FOLLOWER: return <UserPlus className="w-3.5 h-3.5 text-white" />;
      case NotificationType.FRIEND_REQUEST: return <UserPlus className="w-3.5 h-3.5 text-white" />;
      case NotificationType.FRIEND_ACCEPT: return <UserCheck className="w-3.5 h-3.5 text-white" />;
      case NotificationType.INCOMING_CALL: return <PhoneIncoming className="w-3.5 h-3.5 text-white" />;
      case NotificationType.NEW_MESSAGE: return <MessageSquare className="w-3.5 h-3.5 text-white" />;
          ...
          groupedNotifications.map((notif) => (
            <div
              ...
              className={`group flex items-start gap-3 p-4 border-b border-gray-50 dark:border-gray-800 cursor-pointer transition-all duration-200
                ${!notif.is_read ? "bg-indigo-50/60 dark:bg-indigo-900/10" : "hover:bg-gray-50 dark:hover:bg-gray-800/50"}
              `}
            >
              {/* Avatar section */}
              <div className="relative shrink-0 mt-1">
                <img
                  src={(notif as any).avatar || (notif as any).sender?.avatar || anhmacdinh.src}
                  alt="Avatar"
