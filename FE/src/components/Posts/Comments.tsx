...
import EmojiPicker, { EmojiClickData } from "emoji-picker-react";
import LikeComment from "./likecomments";
import { useSocket } from "../SocketContext"; // 1. Import Socket
...
export default function CommentForm({
  ...
  // 3. Tách hàm loadComments ra dùng useCallback để tái sử dụng
  ...
  // Gọi loadComments khi mount
  useEffect(() => {
    loadComments();
  }, [loadComments]);

  // 4. Lắng nghe Socket Realtime
  useEffect(() => {
    if (!socket) return;

    const handleNewNotification = (notif: any) => {
      // Kiểm tra nếu là thông báo COMMENT MỚI
      if (notif.type === "NEW_COMMENT") {
        // Kiểm tra xem thông báo này có thuộc về bài viết hiện tại không
        // Logic parse ID bài viết từ resource_url (tương tự file Feed)
        let notifPostId = notif.resource_id; // Mặc định
        console.log("notif",notif)
        // Nếu resource_url có dạng /posts/123 thì ưu tiên lấy ID từ đó
        ...
        // Nếu đúng là bài viết này thì load lại comment
        if (Number(notifPostId) === Number(postId)) {
           console.log(`🔔 [CommentForm] Nhận bình luận mới ở Post ${postId}, đang tải lại...`);
           loadComments(); // Tự động reload comment
           onCommentAdded?.(); // Cập nhật số lượng comment ở ngoài Feed
        }
      }
    };
